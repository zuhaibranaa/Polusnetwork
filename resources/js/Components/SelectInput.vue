<template>
    <div class="space-y-1">
      ...
      <span class="inline-block w-full rounded-md shadow-sm">
        <button type="button" @click="handleDropDown" aria-haspopup="listbox" aria-expanded="true"
          ...
          <div class="flex items-center space-x-3">
              <img :src="images[value]" alt="value" class="flex-shrink-0 h-6 w-6 rounded-full"/>
          ...
          <span class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
            ...
            <div v-show="isOpen" class="absolute mt-1 w-full rounded-md bg-white shadow-lg">
                <ul tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-item-3"
                    ...
                    <li tabindex="0" @click="select(d)" id="listbox-item-0" role="option" v-for="d in data"
                        v-bind:key="d"
                        class="z-10 text-gray-900 cursor-default select-none relative py-2 pl-3 pr-9  cursor-pointer hover:text-white hover:bg-indigo-600 focus:outline-none focus:text-white focus:bg-indigo-600">
                        <div class="flex items-center space-x-3">
                            <img :src="images[d]" alt="" class="flex-shrink-0 h-6 w-6 rounded-full"/>
